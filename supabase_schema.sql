...
-- Depending on your architecture, notifications might be created by triggers or backend logic.
-- If created by client (not recommended but simple), you need an INSERT policy.
-- A better approach is using Database Webhooks or Triggers, but for this clone, we'll allow authenticated users to insert notifications they 'caused'.
CREATE POLICY "Users can create notifications they triggered." ON public.notifications
