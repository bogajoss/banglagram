  ...
  return (
    <div className="w-full max-w-[630px] pt-0 md:pt-[30px] flex gap-16 flex-col">
      <div className={`md:hidden sticky top-0 z-10 border-b ${borderClass} px-4 h-[60px] flex items-center justify-between ${theme === 'dark' ? 'bg-black' : 'bg-white'}`}>
        ...
        <div className="flex items-center gap-5"><Heart size={24} onClick={() => showToast('নোটিফিকেশন')} /><div className="relative" onClick={() => navigate('/messages')}><MessageCircle size={24} /><div className="absolute -top-1 -right-1 bg-[#f42a41] text-[10px] w-4 h-4 rounded-full flex items-center justify-center font-bold text-white">1</div></div></div>
      </div>
      <div className="flex-grow w-full max-w-[470px] mx-auto px-0 md:px-0">
        <div className="flex gap-4 mb-6 overflow-x-auto scrollbar-hide py-4 px-4 md:px-0">
          {stories.map((story: Story) => (
            <div key={story.id} className="flex flex-col items-center gap-1 cursor-pointer flex-shrink-0 w-[66px] group" onClick={() => setViewingStory(story.id)}>
        ...
        <div className="flex flex-col gap-4 pb-20">
          {posts.map((post: Post) => <PostItem key={post.id} post={post} isSaved={savedPostIds.has(post.id)} onToggleSave={() => toggleSave(post.id)} onUserClick={handleUserClick} onPostClick={setViewingPost} />)}
        </div>
      </div>
      <div className="hidden lg:block w-[319px]">
         ...
         <div className="flex flex-col gap-3">
            {initialData.suggestedUsers.slice(0,5).map((u,i) => (
               <div key={i} className="flex items-center justify-between hover:bg-white/5 p-2 rounded-lg transition-colors cursor-pointer" onClick={() => handleUserClick(u as User)}>
                  <div className="flex items-center gap-3">
